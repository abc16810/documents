...
$$\left\{  \begin{matrix}    3 \\   6 \end{matrix}  \right\}\left\{  \begin{matrix}    3 \\    4     \\    6 \end{matrix}  \right\} $$  $$ \left\{  \begin{matrix}    x_1 \\    x_2  \\    \vdots   \\    x_n \end{matrix}  \right\}$$ 
...
若使用爱因斯坦求和约定（Einstein summation convention），矩阵$A$, 相乘得到矩阵$C$可以用下式表示： $$ a_{ik}*b_{kj}=c_{ij} \tag{1.3-1} $$ 其中，$a_{ik}$, , 分别表示矩阵$A, B, C$的元素，$k$出现两次，是一个哑变量（Dummy Variables）表示对该参数进行遍历求和。 而矩阵和向量相乘可以看成是矩阵相乘的一个特殊情况，例如：矩阵$B$是一个$n \times 1$的矩阵。
...
![33](\img\微信截图_20190918132809.png)
