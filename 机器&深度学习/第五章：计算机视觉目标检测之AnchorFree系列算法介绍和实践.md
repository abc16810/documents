...
- 1、生成anchor
  ...
  这里可以认为是生成Anchor先验，RT-DETR认为目标更多的集中在中心区域，即中心区域的特征更容易产生高，而到了最后一个特征图，由于语义特征非常丰富，所以近似会认为所有特征都很有用，故而全部选择了
  ![](./imgs/d31a05bae73a4a1546623321dc1c3802.png)
- 2、Topk 查询向量选择
...
RT-DETRv2的框架与RT-DETR相同，只是对解码器的可变形注意模块进行了修改。

- 不同尺度的采样点个数不同： 当前的DETRS利用可变形的注意力模块（Zhu等[2020]）来缓解由多尺度特征组成的长序列输入所造成的高计算开销。RT-DETR解码器保留了这个模块，它在每个尺度上定义了相同数量的采样点。我们认为这种约束忽略了特征在不同尺度上的内在差异，限制了可变形注意力模块的特征提取能力。因此，我们建议针对不同的尺度设置不同的采样点个数，以达到更加灵活和高效的目的
- 离散采样：提高RT-DETR的实用性，使其无处不在。我们重点比较了yolo和RT-DETR的部署需求，其中RT-DETR特定的`grid_sample`操作符限制了其广泛的适用性。因此，我们提出了一个可选的`discrete_sample`运算符来代替`grid_sample`，从而消除了RT-DETR的部署约束。具体来说，我们对预测的采样偏移量进行舍入运算，省去了耗时的双线性插值。然而，舍入操作是不可微的，因此我们关闭用于预测采样偏移的参数的梯度。在实践中，我们首先使用`grid_sample`算子进行训练，然后将其替换为`discrete_sample`算子进行微调。对于推理和部署，模型使用了`discrete_sample`算子

数据

- 动态数据增强：为了使模型具有鲁棒的检测性能，我们提出了动态数据增强策略。考虑到检测器在训练前期泛化能力较差的问题，我们对检测器进行了较强的数据增强，而在训练后期，我们降低了检测器的数据增强水平，使检测器适应目标域的检测。具体来说，我们保持了早期的RT-DETR数据增强，同时关闭
  `randomphotometricdistortion, RandomZoomOut， randomoucrop和MultiScaleInput`在最后的两轮
- 自适应规模超参数定制: 我们还观察到，不同大小的缩放后的`RT-DETRs`使用相同的优化器超参数进行训练，导致它们的次优性能。因此，我们提出了缩放`RT-DETRs`的自适应超参数定制。考虑到轻量检测器（如ResNet18 (He et al.[2016])）的预训练主干具有较低的特征质量，我们提高了其学习率。相反，具有大型检测器的预训练主干（如ResNet101 (He et al.[2016])）具有更高的特征质量，我们降低了其学习率。

与RT-DETR一样，我们使用在ImageNet上预训练的ResNet （He et al.[2016]）作为主干，并使用
AdamW （Loshchilov and Hutter[2018]）优化器，批量大小为16，并应用`ema_decay = 0.9999`的指数移动平均（EMA）。对于可选的离散采样，我们首先使用`grid_sample`算子预训练6x，然后使用`discrete_sample`算子微调1x。对于自适应规模的超参数定制，超参数如表1所示，其中lr表示学习率


| Model       | Backbone  | lrbackbone | lrdet |
| ----------- | --------- | ---------- | ----- |
