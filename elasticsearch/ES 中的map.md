...
| analyzer        | 分析器参数指定索引或搜索文本字段时用于文本分析的分析器。这个分析器将同时用于索引和搜索分析，如果指定search_analyzer，覆盖搜索分析器 |
| fields          | 为不同目的以不同方式索引同一字段通常很有用，这就是*multi-fields*的目的。例如，字符串字段可以映射为全文搜索的文本字段，以及排序或聚合的关键字字段 |
